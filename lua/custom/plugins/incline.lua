...
vim.pack.add { { src = gh 'b0o/incline.nvim' }, { src = gh 'nvim-tree/nvim-web-devicons' } }
  ...
  render = function(props)
    ...
    local function get_git_diff()
      ...
      for name, icon in pairs(icons) do
        if tonumber(signs[name]) and signs[name] > 0 then table.insert(labels, { icon .. signs[name] .. ' ', group = 'Diff' .. name }) end
      end
